...
def main():
    ...
    ap.add_argument('--out',default='results/polar_screen_v1');ap.add_argument('--steps',type=int,default=2000)
    ap.add_argument('--seconds',type=float,default=300);args=ap.parse_args()
    ...
    plan={'dataset':'Salesforce/wikitext wikitext-103-raw-v1','dataset_manifest_sha256':digest(root/'manifest.json'),
          ...
          'methods':['real_fp32','real8','polar_fp32','mag4phase4'],'seeds':[0,1,2],
          'gate':'Mean paired validation NLL of mag4phase4 no more than 0.05 above real8 and polar_fp32; real8 mean NLL below 6.0 to reject undertrained screens; all jobs completed with exact deployment checks. This is an exploratory screening threshold, not statistical significance.',
    ...
    for seed in plan['seeds']:
        for method in plan['methods']:
            ...
            job=out/f'{method}_seed{seed}';job.mkdir();history=[];best=float('inf');best_step=0;target_count=0
            ...
            row={'method':method,'seed':seed,'parameters':sum(p.numel() for p in model.parameters()),
                 ...
                 'train_targets_presented':target_count,'seconds':time.monotonic()-job_started,
    ...
    gaps={}
    if complete:
        for control in ['real8','polar_fp32']:
            gaps[control]=statistics.mean(lookup['mag4phase4',s]-lookup[control,s] for s in plan['seeds'])
    gate={'completed_jobs':len(rows),'planned_jobs':12,'budget_exhausted':budget_hit,
          'total_wall_seconds':time.monotonic()-started,'mean_paired_nll_gaps':gaps,
          'passed':complete and all(g<=.05 for g in gaps.values()) and statistics.mean(lookup['real8',s] for s in plan['seeds'])<6.0,'action':'No full-data job is launched by this screen.'}
    atomic_json(out/'gate.json',gate)
    text='# Small real-data magnitude/phase screen\n\n'
