...
def main() -> None:
    ...
    ap.add_argument("--gpu-gib", type=int, default=17)
    ...
    full = (len(ids) - 1) // 128
    blocks = [ids[i * 128 : i * 128 + 129] for i in range(full)][: a.eval_blocks]
    ...
    res = {"plan": {"model": a.model, "target_layers": layers, "tensor_shape": list(shape0),
                    "target_params": n_tgt_params, "total_params": total_params,
    ...
    for name, dim, k, group in ARMS:
        ...
        for i in layers:
            ...
            del w, idx, sc, ref, dq
            torch.cuda.empty_cache()
        assert maxdiff == 0.0, f"{name}: decoded weights differ by {maxdiff}"
    ...
    refs = ("scalar3_rot_gptq", "scalar3_g64_rot_gptq")
    comps = {}
    for m in res["arms"]:
        for ref in refs:
    ...
    L = ["# Scale transfer: the section 5c recipe on Qwen3.8-27B", "",
         ...
         "split. Every arm asserts decode(encode(w)) bit-exact.", "",
