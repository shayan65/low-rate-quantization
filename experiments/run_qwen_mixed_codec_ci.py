"""Layer-wise polar/GPTQ mixed codec with paired block bootstrap intervals."""
...
def bootstrap(blocks,reference,seed=92016,reps=5000):
 a=np.array([x['loss_sum'] for x in blocks]);b=np.array([x['loss_sum'] for x in reference]);c=np.array([x['count'] for x in blocks]);rng=np.random.default_rng(seed);vals=[]
...
def main():
 ...
 val_ids=tok('\n'.join(pd.read_parquet('data/tinystories/validation.parquet').text.tolist()),add_special_tokens=False).input_ids[:261285];windows=[val_ids[s:min(s+129,len(val_ids))] for s in range(0,len(val_ids)-1,128)]
 selections=json.loads(Path('results/qwen_tinystories_matched_v1/selections.json').read_text());model=Qwen3_5ForCausalLM.from_pretrained(args.model,dtype=torch.bfloat16,device_map='cuda').eval();layers=[x['layer'] for x in selections]
 ...
 loss_on_windows(model,cal,started,args.seconds,'mixed_capture')
 ...
 plan={'model':'Qwen/Qwen3.5-0.8B-Base','dataset':'TinyStories matched 261,284-target protocol','selection':'per layer, choose task-selected polar or block-GPTQ by train-only calibration NLL','codec_metadata':'one byte per tensor encodes codec and pairing; counted','methods':['bf16','task_real4','task_polar3_5','block_gptq_real4','mixed_polar_gptq'],'uncertainty':'5,000 paired bootstrap replicates over contiguous 128-target blocks','runtime_cap_seconds':args.seconds};atomic_json(out/'plan.json',plan);(out/'run_qwen_mixed_codec_ci.py').write_bytes(Path(__file__).read_bytes())
 for sel in selections:
  l=sel['layer'];p=model.model.layers[l].linear_attn.in_proj_qkv.weight;w=originals[l].to('cuda');qp,_,_,_=polar35(w,sel['polar_pairing']);p.data.copy_(qp.to(torch.bfloat16));pn=loss_on_windows(model,cal,started,args.seconds,f'mixed_polar_l{l}')['nll'];qg,_,_=block_gptq(w,captures[l]);p.data.copy_(qg.to(torch.bfloat16));gn=loss_on_windows(model,cal,started,args.seconds,f'mixed_gptq_l{l}')['nll'];p.data.copy_(w.to(torch.bfloat16));codec='polar' if pn<=gn else 'gptq';mixed.append({'layer':l,'codec':codec,'polar_calibration_nll':pn,'gptq_calibration_nll':gn,'polar_pairing':sel['polar_pairing'],'polar_id':sel['polar_id']});atomic_json(out/'mixed_selections.json',mixed);print(json.dumps(mixed[-1]),flush=True)
 methods=plan['methods'];rows=[];all_blocks={}
 for method in methods:
  ...
  if method!='bf16':
   for sel,choice in zip(selections,mixed):
    ...
    if chosen=='task_real4':q,payload=payload_real(w,sel['real_factor']);blob=bytes([sel['real_id']])+payload
    elif chosen=='task_polar3_5':q,payload=payload_polar(w,sel['polar_pairing']);blob=bytes([sel['polar_id']])+payload
 ...
 ci={m:bootstrap(all_blocks[m],all_blocks['bf16']) for m in methods[1:]};atomic_json(out/'confidence_intervals.json',ci);base=rows[0]['validation']['nll']
 for r in rows:r['delta_nll_vs_bf16']=r['validation']['nll']-base;r['paired_ci95']=ci.get(r['method'],{'ci95':[0,0]})['ci95']
 atomic_json(out/'results.json',rows);gate={'complete':len(rows)==5,'seconds':time.monotonic()-started,'polar_layers':sum(x['codec']=='polar' for x in mixed),'gptq_layers':sum(x['codec']=='gptq' for x in mixed),'mixed_beats_polar':rows[4]['validation']['nll']<rows[2]['validation']['nll'],'mixed_beats_gptq':rows[4]['validation']['nll']<rows[3]['validation']['nll']};atomic_json(out/'gate.json',gate)
 text='# Mixed polar/GPTQ codec with paired confidence intervals\n\n| Method | NLL | Delta NLL (95% paired block CI) | Export bytes |\n|---|---:|---:|---:|\n'
 for r in rows:text+=f"| {r['method']} | {r['validation']['nll']:.6f} | {r['delta_nll_vs_bf16']:+.6f} [{r['paired_ci95'][0]:+.6f}, {r['paired_ci95'][1]:+.6f}] | {r['payload_bytes']:,} |\n"
