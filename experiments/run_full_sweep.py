...
from run_full_lm import atomic_json, digest, METHODS
...
def collect(root):
    ...
    for job in plan['jobs']:
        ...
        assert r['source_sha256']==config['source_sha256']==digest(directory/'run_full_lm.py')
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--root',default='results/wikitext103_full_v1')
    ...
    training=Path(__file__).with_name('run_full_lm.py').resolve()
    # First seed prioritizes user's representation and matched controls.
    hybrid=['continuous','restricted180','full180','full256','real_fp32','real2','real4','real8','complex_fp32','phase4','phase8','phase16']
    transformer=['real_fp32','continuous','restricted180','full180','full256']
    jobs=[]
    ...
    plan={'dataset':'Salesforce/wikitext','subset':'wikitext-103-raw-v1','epochs':args.epochs,'seeds':args.seeds,
          ...
          'scope':'Full real-data language-model quantization comparison; not the temporal-calibration or Qwen experiments','jobs':jobs}
    ...
    (root/'run_full_sweep.py').write_bytes(Path(__file__).read_bytes())
    ...
    for job in jobs:
        ...
        command=[sys.executable,str(training),'--data',args.data,'--out',str(directory),
            ...
            '--epochs',str(args.epochs),'--width','384','--depth','6','--heads','6','--context','512','--batch','16']
