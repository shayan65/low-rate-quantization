    ...
    with open(directory/'projections.bin','wb') as f:
        for name,module in model.named_modules():
            ...
            if method.startswith('mag'):
                ...
                bits=int(method[4:]); clip=module.log_clip.exp().detach()
                ix=((module.weight.detach()/clip).clamp(-1,1)+1)*((2**bits-1)/2)
                payload=struct.pack('<f',float(clip))+pack_indices(ix.round().long().cpu().numpy(),bits)
...
def main():
    ...
    ap.add_argument('--allow-full-run',action='store_true',help='Explicitly enable full-data training after limited validation')
    args=ap.parse_args()
    if args.verify: verify(); return
    if args.prepare: prepare(args.data,args.subset,args.vocab); return
    if not args.allow_full_run:
        raise SystemExit('Full-data training disabled by default. Use the time-limited sample screen; no GPU model allocated.')
