...
METHODS = ['real_fp32', 'real2', 'real4', 'real8', 'complex_fp32', 'continuous',
           'restricted180', 'full180', 'full256', 'phase4', 'phase8', 'phase16']
...
class Projection(nn.Module):
    def __init__(self, din, dout, method):
        ...
        if method in ['continuous','restricted180','full180','full256','phase4','phase8','phase16']:
    ...
    def coefficients(self):
        ...
        if self.method in ['real2','real4','real8']:
            ...
            w=c*(normalized+(q-normalized).detach())
...
def export_model(model,directory):
    ...
    with open(directory/'projections.bin','wb') as f:
        for name,module in model.named_modules():
            ...
            if hasattr(module,'phase') and method!='continuous':
...
def train(args):
    ...
    if (out/'config.json').exists():
        ...
        atomic_json(out/'config.json',config); (out/'run_full_lm.py').write_bytes(Path(__file__).read_bytes())
        ...
        for start in range(begin,train_blocks,args.batch):
            ...
            if global_step%args.log_every==0 or global_step==1:
                ...
                status={'event':'train','epoch':e+1,'step':global_step,'total_steps':total_steps,
                    ...
                    'elapsed_seconds':seconds,'lr':lr,'tokens_per_second_this_process':train_loss_tokens/max(.001,time.time()-train_started),
    ...
    assert torch.allclose(before,after,atol=.02,rtol=.01), 'Deployment roundtrip changed logits'
