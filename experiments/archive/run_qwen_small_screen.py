...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--model',default='models/Qwen3.5-0.8B-Base');ap.add_argument('--out',default='results/qwen_small_v1');ap.add_argument('--seconds',type=float,default=180);ap.add_argument('--sequences',type=int,default=8);ap.add_argument('--sample-seed',type=int,default=20260916);args=ap.parse_args()
 ...
 rng=np.random.default_rng(args.sample_seed);starts=np.sort(rng.choice(len(ids)-129,args.sequences,replace=False));windows=[ids[s:s+129] for s in starts]
 ...
 plan={'model':'Qwen/Qwen3.5-0.8B-Base','parameters':sum(x.numel() for x in model.parameters()),'tensor':'model.layers.0.linear_attn.in_proj_qkv.weight','shape':list(p.shape),
  'dataset':'local real WikiText-2 validation parquet','sample_seed':args.sample_seed,'sample_starts':starts.tolist(),'sequences':args.sequences,'sequence_tokens':129,'target_tokens':args.sequences*128,
 ...
 text=f'# Qwen3.5-0.8B one-projection screen\n\nFrozen model, one DeltaNet input projection, {args.sequences*128:,} real WikiText-2 validation targets. This is not full-dataset perplexity.\n\n| Method | NLL | Perplexity | Delta NLL | Payload bytes |\n|---|---:|---:|---:|---:|\n'
